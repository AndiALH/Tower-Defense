...
public class Bullet : MonoBehaviour
{
    ...
    private ElementType _bulletElement;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.Equals(_targetEnemy.gameObject))
        {
            ...
            {
                LevelManager.Instance.ExplodeAt(transform.position, _bulletSplashRadius, _bulletPower, _bulletElement);
            }
            // Bullet yang hanya single-target
            else
            {
                _targetEnemy.ReduceEnemyHealth(_bulletPower, _bulletElement);
            }
            _targetEnemy = null;
        }
    }

    public void SetProperties(int bulletPower, float bulletSpeed, float bulletSplashRadius, ElementType bulletElement)
    {
        _bulletPower = bulletPower;
        _bulletSpeed = bulletSpeed;
        _bulletSplashRadius = bulletSplashRadius;
        _bulletElement = bulletElement;
    }
